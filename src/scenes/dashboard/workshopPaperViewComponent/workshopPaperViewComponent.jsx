...
const WorkshopPaperViewPage = () =>
{
	...
	{
		communicationService.getWorkshop(null,
			(res) =>
			{
				const { data } = res.data;
				...
				data.user.map((item) =>
				{
					rowData.push(
						createData(item._id,
							item.publisherName, item.title, item.publishDate, item.fileName),
					);
	...
	return (
		<>
			...
			<Card>
				<CardHeader
					subheader='All the workshop paper submissions '
					title='Workshop Paper Review form'
				/>
				<Divider />
				<CardContent>
					<TableContainer component={Paper}>
						<Table sx={{ minWidth: 650 }} aria-label='simple table'>
							<TableHead>
								<TableRow>
									<TableCell>Workshop Conductor</TableCell>
...
export default WorkshopPaperViewPage;
